...
- After implementation, confirm the behaviour is present yourself, then add the comments and JSDoc the change needs per `code-style` (purpose-first on every declaration and function, why as a guardrail only where intent could look like a bug), then send the Reviewer to run `project-review-worktree`. Gate only on must-fix findings, failed applicable craftsmanship standards, or missing load-bearing evidence. Keep recommendations and nice-to-haves visible to the human, without automatically sending another implementation cycle.
- The comment/JSDoc pass must produce a checkable artefact, not a silent skim: list every new or touched docstring in the handoff and confirm each one states purpose, not mechanics, before dispatching to the Reviewer. Do not send a chunk to review without that list.
