...
- A completion report without the named Scout's terminal verification receipt is invalid. Local inability to run a project evidence command is not a verification blocker while that Scout is available. Do not copy the full Scout receipt; report its result and log reference.
...
```sh
hcom send @<exact-requester-name> --intent inform --reply-to <assignment-id> --thread <episode-thread> -- Implemented <task>. Safe to reset: yes. Changed <paths>. Code prose: none added or edited. Scout <exact-name>: <checks summary>; diagnostic <log reference>. Remaining concern: <none or detail>.
```
