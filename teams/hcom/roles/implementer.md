...
- Read the task and exact implementation references yourself. The Implementer changes files and never runs project discovery, diagnostics, tests, lint, check-only formatters, diff checks, or repro commands. After editing, send every foreseeable verification outcome to the exact Scout named in the assignment as one batch, finish independent inspection of your changed lines while Scout runs, then wait for its terminal receipt. If required verification has no exact Scout identity, ask the Orchestrator once before editing.
