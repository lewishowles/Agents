...
- If a command is blocked by the environment, don't retry equivalent variants. Finish any independent in-scope work, then include all blocked commands and errors from that step in one blocker or completion report.
...
- Before contacting the Orchestrator or Scout for a phase, collect every foreseeable question, decision, and verification command into one packet. Do not send questions or commands one at a time. Follow up only when the reply reveals a genuinely new blocker or requested correction.
