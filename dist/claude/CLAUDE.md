...
**Distil before closing.** Before updating the handoff, ask what was learned: what belongs in `## Discoveries`, what belongs in `## Decisions`, and whether any dead ends should be recorded as `### Failed approaches`. Add only what isn't already captured. Identify whether the completed work produced a verified, durable fact that applies to most future sessions and changes the agent's default action. If so, propose its smallest `AGENTS.md` entry with the fact, scope, required action, and evidence; promote it after approval. Keep feature-specific, temporary, or unproven findings in `PROGRESS.md`, a spec, or focused documentation instead.
...
**PROGRESS.md update is blocking only at task boundaries.** At a task-boundary event defined below, update the `PROGRESS.md` handoff before stopping. An accepted interim commit in a still-open multi-commit task is not that event. A multi-commit task's `## Commit plan` uses `- [ ] Commit N: outcome` entries; tick one only when the user explicitly accepts that commit's handoff. These interim-commit acceptance states are distinct from the detailed implementation checkboxes under `## Tasks`. Change a `ready` task to `in-progress` when work starts on its first unchecked commit-plan entry. Keep it `in-progress` until the final entry is accepted, then archive the task directly in `PROGRESS.md`, remove its queue entry, promote the next task, and trash the task file. Do not add a transient `done` status, `completed` date, or `## Outcome` section to a file being removed. This is a user-handoff decision, not a Git-state check.

**One write per task-boundary event.** Update `PROGRESS.md` once at completion or acceptance, consolidating all section changes (handoff, discoveries, decisions, queue, archive) into a single Edit/Write call, not one call per section. Do not edit it mid-task as a progress checkpoint. A task-boundary event is the task file's own completion or the user's acceptance of it, not each internal commit inside a still-open multi-commit task — a reviewer-approved commit that leaves the task `in-progress` is not itself a write trigger. Carry interim commit-by-commit state in the session's own handoff to the user (e.g. an hcom message) instead. The exception is a durable interruption — a material scope change, a blocker, or a user decision that changes the next session's safe action — which is worth recording immediately, since it prevents a future session from repeating costly investigation.
...
Where the runtime supports forking, prefer it over a fresh agent when the subtask needs the current conversation's context: a fork inherits that context and shares the prompt cache, so it costs far less than a fresh agent re-deriving the same context from cold. Reserve fresh dispatch for cases that need genuine independence, as in the Review gate below.
