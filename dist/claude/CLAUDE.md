...
**Store HCOM Orchestrator handoffs before closing.** Every HCOM Orchestrator close replaces the current project's handoff with `progress context set`. Supply all six fields: `--current-goal`, `--previous-step`, `--next-step`, `--standing-context`, `--verify-with`, and `--stop-marker`; omitted fields are cleared. Keep the human response to the result or blocker, one next action, and any required suggested commit message. Do not repeat details stored in the handoff. Tell the next agent to retrieve the full handoff with `progress context get --json`; do not imply that `progress context set` retrieves it. Do not say that the current team remains available because closing the Orchestrator closes the team.
