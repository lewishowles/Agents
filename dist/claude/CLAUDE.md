...
- Never run a full Playwright or Cypress suite, including through diagnostics. Diagnostics controls output volume, not execution time. Run only specific browser test files, and ask the user to run broad browser suites. Don't trust a diagnostics tool's `--all`/broad-run eligibility for a script that's Playwright/Cypress-backed: the ban applies regardless of tool classification. Scoping test files doesn't guarantee a scoped build: check for a shared component-test build config before assuming a multi-file run is cheap.
...
**One write per task-boundary event.** Update `PROGRESS.md` once at completion or acceptance, consolidating all section changes (handoff, discoveries, decisions, queue, archive) into a single Edit/Write call, not one call per section. Do not edit it mid-task as a progress checkpoint. The exception is a durable interruption — a material scope change, a blocker, or a user decision that changes the next session's safe action — which is worth recording immediately, since it prevents a future session from repeating costly investigation.
