...
2a. **Tool-error pass (mandatory, every run)** - tool-error rows are auto-logged per-command failures (via tool-failure-log.sh) and excluded from the default analyser output because their detail text is almost always unique, making the count column meaningless for this category. Every run must additionally invoke the analyser with FRICTION_INCLUDE_TOOL_ERRORS=1 and manually cluster the results by error-message substring or command shape (not by the count column) to find real repeating failure signatures. Treat a cluster the same as any other pattern step 2 covers.
