...
1. **Run the analyser** — `bash scripts/analyse-friction.sh`. Output is `count ⇥ category ⇥ cwd ⇥ detail`, sorted most-frequent-first. By default it combines the central friction log with project-local fallback logs under `$HOME/Dev` and excludes automated `check-fail` rows; set `FRICTION_INCLUDE_CHECK_FAILS=1` only when reviewing verification debt. Pass explicit log paths only for a scoped review.
2. **Take the top recurring patterns** — entries with count ≥ 2 are worth reviewing; a single occurrence is usually not yet a pattern. Group entries that describe the same underlying failure even if `cwd`/`detail` differ slightly. Treat `check-fail` rows as verification debt, not rule-change evidence, unless the detail shows a repeated agent behaviour that existing check-running guidance would not have prevented.
