...
Root `PROGRESS.md`, when present, tracks active maintenance session state; root `AGENTS.md` is maintenance guidance (not generated). It is intentionally absent between maintenance plans. Do not report it missing or create it as follow-up unless the current task needs an active plan. See `project-continue` and `project-setup` skills for session-resume conventions and schema. Workspace files generated by `scripts/init-workspace.py`.
