...
[
	...
	{"event":"PostToolUseFailure","matcher":"","command":"cmd=${HCOM:-hcom}; command -v \"${cmd%% *}\" >/dev/null 2>&1 && exec $cmd post-failure || exit 0","timeout":null},
	...
	{"event":"StopFailure","matcher":"","command":"cmd=${HCOM:-hcom}; command -v \"${cmd%% *}\" >/dev/null 2>&1 && exec $cmd stop-failure || exit 0","timeout":null},
