...
def generated_guard(project_dir: Path) -> dict[str, Any]:
	script = existing_script(project_dir, "generated-file-guard.py")
	if not script:
		return {"available": False, "findings": [], "ok": True}

	exit_code, output = command_output([str(script), "--project-dir", str(project_dir), "--json"], project_dir)
	try:
		result = json.loads(output)
	except json.JSONDecodeError:
		return {"available": False, "findings": [], "ok": exit_code == 0}
